...
func update_points():
	...
	while curr_line_len < Vector2().distance_to(final_goal):
		var move_vector = (
			start_point.direction_to(final_goal)
			* rand_range(min_segment_size, max_segment_size)
		)
		var new_point = start_point + move_vector
		var new_point_rotated = (
			start_point
			+ move_vector.rotated(deg2rad(rand_range(-angle_var, angle_var)))
		)
